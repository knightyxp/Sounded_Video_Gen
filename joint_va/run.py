...
import sys 
sys.path.append('/home/yxingag/sear/AudioLDM')
sys.path.append('/home/yxingag/sear/AnimateDiff')
...
my_model_path = '/home/yxingag/sear/AudioLDM/ckpt/audioldm-m-full'
...
parser.add_argument("--prompt_txt",      type=str, default="1021_video_audio_eval.txt")    
...
prompt_list = []
with open(animatediff_args.prompt_txt, 'r') as f_open:
    for line in f_open:
        prompt_list.append(line.strip())

prompt_list = prompt_list[animatediff_args.start:animatediff_args.end] 
print('all promots: ', prompt_list) 
...
clip_duration = 1
clips_per_video = 1
savedir = savedir + f'_inferstep{num_inference_steps[0]}_lr{learning_rate[0]}'
...
for prompt in prompt_list:
    for n_step in num_inference_steps:
        ...
        for sample_id in range(animatediff_args.n_sample_per_prompt):
            audio_org = audio_pipe(prompt, num_inference_steps=n_step, audio_length_in_s=2.0, generator=generator).audios[0]
            ...
            sf.write(f"{savedir}/org_{prompt_to_save}_{sample_id:02d}.wav", audio_org, samplerate=16000) 
            save_videos_grid(video_org, f"{savedir}/org_{prompt_to_save}_{sample_id:02d}.gif") 

            # save video + audio 
            audio = AudioFileClip(f"{savedir}/org_{prompt_to_save}_{sample_id:02d}.wav")
            video = VideoFileClip(f"{savedir}/org_{prompt_to_save}_{sample_id:02d}.mp4")
            video = video.set_audio(audio)
            video.write_videofile(f"{savedir}/org_{prompt_to_save}_{sample_id:02d}_combine.mp4", codec="libx264")                
            ...
            for lr in learning_rate:
                ...
                for cur_step in range(n_step):
                    with torch.autograd.set_detect_anomaly(True):
                        ...
                        optimizer_video = torch.optim.Adam([xt_video_temp], lr=lr) 
                        optimizer_audio = torch.optim.Adam([xt_audio_temp], lr=lr) 
                ...
                sf.write(f"{savedir}/ours_{prompt_to_save}_{sample_id:02d}.wav", audio, samplerate=16000)
                save_videos_grid(video, f"{savedir}/ours_{prompt_to_save}_{sample_id:02d}.gif")

                audio = AudioFileClip(f"{savedir}/ours_{prompt_to_save}_{sample_id:02d}.wav")
                video = VideoFileClip(f"{savedir}/ours_{prompt_to_save}_{sample_id:02d}.mp4")
                video = video.set_audio(audio)
                video.write_videofile(f"{savedir}/ours_{prompt_to_save}_{sample_id:02d}-combine.mp4", codec="libx264")
