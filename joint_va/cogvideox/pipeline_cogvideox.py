...
class CogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def bind_step(
        ...
        print('device',device)
        print('self.transformer',self.transformer.device)
        # Denoising loop
    ...
    def decode_latents_bind(self, latents):
        print('video_latents',latents.shape)
        # latents b ,t ,c ,h ,w
        latents = latents.permute(0, 2, 1, 3, 4)  # [batch_size, num_channels, num_frames, height, width]
        latents = 1 / self.vae_scaling_factor_image * latents
    ...
    def bind_finish(
        ...
        # Post-processing
        video = self.decode_latents(latents)
        video = self.video_processor.postprocess_video(video=video, output_type='numpy')
        ...
        return CogVideoXPipelineOutput(videos=video)
