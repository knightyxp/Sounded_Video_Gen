...
BPE_PATH = "/home/yazhou/disk1/projects/edit/joint-generation/imagebind/bpe/bpe_simple_vocab_16e6.txt.gz"
...
def load_and_transform_video_data(
    ...
    for video_path in video_paths:
        ...
        for i_time, clip_timepoints in enumerate(all_clips_timepoints):
            ...
            print('video_clip: ', video_clip.max(), video_clip.min())
            imageio.imwrite(f'intermediate/{video_name}/sampled_frame_{clip_duration}_{clips_per_video}_{(i_time*2):02d}_0.jpg', np.uint8(video_clip[:, 0, :, :].permute(1, 2, 0).numpy()))
        ...
        for i_save in range(len(all_video_save)):
            print('all_video: ', all_video_save[i_save].max(), all_video_save[i_save].min())
            imageio.imwrite(f'intermediate/{video_name}/sampled_frame_0_crop_{clip_duration}_{clips_per_video}_{i_save:02d}.jpg', np.uint8(all_video_save[i_save][:, 0, :, :].permute(1, 2, 0).numpy()*255.0))
            # imageio.imwrite(f'temp/{video_name}_sampled_frame_1_crop_{i_save:02d}.jpg', all_video[i_save][:, 1, :, :].permute(1, 2, 0).numpy())
...
def my_uniform_sampler(video_tensor, num_samples):
    ...
    # uniform sampling
    if video_length < num_samples:
        num_samples = video_length
...
def load_and_transform_video_data_from_tensor_real(
    ...
    clips_per_video=5,
    sample_rate=16000,
    n_samples_per_clip=2, # num_frames per clips
    video_fps = 10,
):
    video_outputs = []
    # requires CTHW 
    ...
    # print(f'clip_duration={clip_duration}, clips_per_video={clips_per_video},video_fps={video_fps},video_duration={video_duration}')
    clip_sampler = ConstantClipsPerVideoSampler(
        clip_duration=clip_duration, clips_per_video=clips_per_video
    )
    frame_sampler = pv_transforms.UniformTemporalSubsample(num_samples=n_samples_per_clip)
    ...
    for vd_tensor in video_tensor:
        for i_time, clip_timepoints in enumerate(all_clips_timepoints):
            ...
            if clip is None:
                raise ValueError("No clip found")
            # print('clip["video"] device: ', clip["video"].device)
            # print('clip["video"] shape: ', clip["video"].shape) # [1, 16, 64, 64, 3]
            # video_clip = frame_sampler(clip["video"])
            video_clip = my_uniform_sampler(clip["video"], num_samples=n_samples_per_clip) # select two frames for the clip
            # print('video_clip: ', video_clip.shape)
            # print('video_clip: ', video_clip.max(), video_clip.min())
            # print('video_clip shape: ', video_clip.shape) # [1, 2, 64, 64, 3]

            # video_clip = video_clip / 255.0  # since this is float, need 0-1

            # print()
            all_video.append(video_clip) # video_clip：0-1
