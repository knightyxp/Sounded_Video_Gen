...
class Audio_Video_LDMPipeline(DiffusionPipeline):
    ...
    def decode_audio_latents(self, audio_latents):
        ...
        if audio_latents_device != self.vae.device:
            multi_gpu = True
        else:
            multi_gpu = False
        if multi_gpu:
        ...
        print(f'self.vae device: {self.vae.device}')
        print('multi gpu:', multi_gpu)
    ...
    def bind_forward_triple_loss(
        ...
        # 8. Post-processing
        # decode audio 
        mel_spectrogram = self.decode_latents(audio_latents)
        ...
        # decode video
        video = self.decode_video_latents(video_latents)
        video = self.video_processor.postprocess_video(video=video, output_type=output_type)
